...
(function () {
  ...
  var BACKEND_URL = window.getBrewButterflyBackendUrl
    ? window.getBrewButterflyBackendUrl(
  ...
  var changePwdForm =
    document.getElementById('change-pwd-form');

  var syncBtn =
    document.getElementById('btn-sync-backend');

  var refreshResBtn =
    document.getElementById('btn-refresh-res');
  ...
  function setAuthUI(isAuthenticated, userName) {
    ...
    window.__bbcAdminAuthenticated =
      isAuthenticated;
    ...
    if (statusBadge) {
      ...
      if (isAuthenticated) {
        ...
        statusBadge.innerHTML =
          '<span class="dot"></span> Sign in to access admin';
      }
    }


    window.dispatchEvent(
      new Event('admin-auth-state-changed')
    );
  ...
  function checkAuth() {
    ...
    .then(function (data) {
      ...
      /*
       ...
       If the backend session is valid, keep the
       existing JWT if we already have one.
      */

      window.BbcAdminSession.set(
        getToken() || 'session',
        username
      );
    ...
    .catch(function () {
      ...
      /*
       Only clear authentication if there is
       NO locally stored JWT.

       This prevents a successful login from
       immediately being wiped out because the
       cookie session endpoint is unavailable.
      */
        ...
        .then(function (data) {
          ...
          */
          ...
          /*
           ...
           Do NOT immediately call checkAuth().

           The previous code called checkAuth()
           immediately after login. If the cookie
           session wasn't recognized by the backend,
           checkAuth() cleared the successful login.
          */
  ...
  if (togglePasswordBtn) {
    ...
    togglePasswordBtn.addEventListener(
      ...
      function () {
        ...
        togglePasswordBtn.classList.toggle(
          'is-visible',
          isHidden
        );
  ...
  if (changePwdForm) {
    ...
    changePwdForm.addEventListener(
      'submit',
      function (e) {
    ...
    syncBtn.addEventListener(
      ...
      function () {
        ...
        if (
          ...
          showToast(
            'Please sign in to sync with backend.',
            'error'
          );
        ...
        .then(function (d) {
          ...
          showToast(
            'Synced ' +
            (d.synced || menu.length) +
            ' menu items to the database.',
            'success'
          );
        })
        ...
        .catch(function (err) {
          ...
          showToast(
            err.message ||
            'Sync failed: backend unreachable.',
            'error'
          );
        })
  ...
  if (refreshResBtn) {
    ...
    refreshResBtn.addEventListener(
      'click',
      function () {
        ...
        .then(function (items) {
          ...
          if (
            ...
            showToast(
              'Reservation list refreshed from backend.',
              'success'
            );
        ...
        .catch(function (err) {
          ...
          if (
            ...
            showToast(
              'Could not refresh reservations from backend.',
              'error'
            );
          }
        });
  ...
  function initAdmin() {
    ...
    /*
     ...
     a login token from this browser tab.
    */
